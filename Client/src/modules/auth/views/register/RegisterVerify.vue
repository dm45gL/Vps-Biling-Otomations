  ...
  <div class="auth-layout">
    <div class="auth-card">
      ...
      <div class="auth-header has-text-centered mb-5">
        <div class="mb-3">
          <i class="fas fa-user-check auth-icon"></i>
        </div>
        <h1 class="auth-title">Verifikasi Akun</h1>
        <p class="auth-desc">
          Kami telah mengirimkan kode OTP ke<br />
          <strong>{{ maskedRegisterEmail || 'email Anda' }}</strong>
      ...
      <form @submit.prevent="handleSubmit" class="auth-form">
        ...
        >
          <span v-if="!loading">Verifikasi Akun</span>
      ...
      <div class="auth-trust mt-5">
        <div class="trust-badge">
          ...
          <span>Kode berlaku 5 menit • Akun terverifikasi</span>
        </div>
      </div>
    </div>
  </div>
...
const resendOtp = async () => {
  if (isResending.value || !isExpired.value) return;

  isResending.value = true;
  const success = await resendRegisterOTP();
...
const handleSubmit = async () => {
  ...
  const clientId = await completeRegistration(otp.value);
  if (clientId) {
    router.push('/profile');
  }
};
...
/* ───────── LIFECYCLE ───────── */
onMounted(() => {
  // Jika session register hilang → balik ke register
  if (!maskedRegisterEmail) {
