  ...
  <div class="auth-layout">
    <div class="auth-card">
      ...
      <div class="auth-header has-text-centered mb-5">
        ...
        <h1 class="auth-title">Verifikasi Login</h1>
        <p class="auth-desc">
          Masukkan kode OTP yang dikirim ke<br />
       <strong>{{ maskedLoginEmail || 'email Anda' }}</strong>
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="auth-form">
        ...
        >
          <span v-if="!loading">Verifikasi & Masuk</span>
...
const handleSubmit = async () => {
  ...
  const clientId = await verifyLogin(otp.value);
...
/* ───────── LIFECYCLE ───────── */
onMounted(() => {
  // 🚨 GUARD: tidak ada session login
  if (!maskedLoginEmail.value) {
